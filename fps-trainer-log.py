...
# ======================================={-lees score bestand json-}==================================================

def score_updaten():

    score_bestand_js = open("highscores.json", "r")
    x=score_bestand_js.readline()
    score_bestand_py = dict(json.loads(x))
    score_bestand_js.close()
# -------------------------------------------

    print(score_bestand_py)

    naam_speler = input("wat is u naam ? : ")

    score_bestand_py[naam_speler] = punten[0]

    gesorteerde_score_bestand = dict(sorted(score_bestand_py.items(),key=lambda x: x[1],reverse= True))

    print(gesorteerde_score_bestand)

# ----------------------------------------------
    score_bestand_js = open("highscores.json", "w")
    tijdelijk_score = json.dumps(gesorteerde_score_bestand)
    score_bestand_js.write(tijdelijk_score[0 : 10])
    score_bestand_js.close()
